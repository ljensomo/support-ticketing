...
<title>Groups - Fortis Ticketing System</title>
				...
				<div class="col-md-12">
					  <div class="panel" style="border-color:#272930;">
					    ...
					    <div class="panel-body">						
					    	<div class="header">
								<a class="btn btn-default" data-target="#add-group" data-toggle="modal">
									...
									?>
										<tr class="odd gradeX">
											...
											<?php 
												...
												$count = "SELECT COUNT(id) FROM group_projects WHERE group_id = ?";
											...
											?>
													...
													<a href="#" type="button" class="label label-danger" style="margin-left:12px;" onclick="remove_project(<?php echo $row_projects[0]; ?>)">
											...
											<?php 
												...
												?>
														<a href="#" type="button" class="label label-danger" style="margin-left:12px;" onclick="remove_user(<?php echo $row_members[0]; ?>)">
															<i class="fa fa-times"></i>
														</a>
														<br /><br />												
											<?php
											...
											<a class="btn btn-warning btn-sm" type="button" onclick="group_modal_show(<?php echo $row_group[0]; ?>,'<?php echo $row_group[1]; ?>')">
											<i class="fa fa-edit"></i>Edit</a>
											<a class="btn btn-danger btn-sm" type="button" id="btn_remove">
...
<script type="text/javascript">
	$('#add_group').on('click',function(){
		...
		if($.trim(group_name) === ''){
			swal('Ooops!','Please input group name!','warning');
		}else{
			$.ajax({
				...
				complete:function(request){
					if(request.responseText.trim() === 'Added!'){
						swal({title:'Save!',text:'Group successfully Added!',type:'success'},
							function(){
								location.reload();
							});
					}else{
						swal('Ooops!',request.responseText.trim(),'warning');
					}
				}
			});
		}
	...
	$('#btn_remove').on('click',function(){
		swal('Remove','Are you sure you want to remove project?','warning');
	});
	function group_modal_show(id,groupname){
		var group_id = id;
	...
	function remove_project(id){
		...
		$.ajax({
			url:'includes/remove_from_group.php',
			type:'post',
			data:{
	...
	$('#btn_save').on('click',function(){
		...
		$.ajax({
			url:'includes/save_group_details.php',
			type:'post',
			data:{
				group_id:group_id,
				groupname:groupname
			...
			complete:function(data){
				if(data.responseText.trim() === 'Updated!'){
					location.reload();
				}else{
					swal('Message',data.responseText,'warning');
				}
			}
		});
	...
	$('#add_project').on('click',function(){
		...
		if(project_id === ''){
			swal('Ooops!.','Please select project to add.','warning');
		}else{
			$.ajax({
				url:'includes/save_group_details.php',
				type:'post',
				...
				complete:function(data){
					if(data.responseText.trim() === 'Added!'){
						swal({title:'Added!',text:'Projects was added.',type:'info'},
								function(){
									location.reload();
								}
							);	
					}else{
						swal('Message',data.responseText,'info');
					}
	...
	$('#add_member').on('click',function(){
		...
		if(user_id === ''){
			swal('Ooops!.','Please select user to add.','warning');
		}else{
			$.ajax({
				url:'includes/save_group_details.php',
				type:'post',
				...
				complete:function(data){
					if(data.responseText.trim() === 'Added!'){
						swal({title:'Message',text:'User was added in the group.',type:'info'},
							function(){
								location.reload();
							}
						);
