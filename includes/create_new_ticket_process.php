...
if($tckt_row[0]>=1){
		...
		if ($error) {
		   ...
		   // openWindow($goto = "../client.php");
		    ...
		    $sqlAdd = "INSERT INTO tickets(project,transaction_id,reporter_id,issue_type) VALUES(?,?,?,?);";
		    $qryAdd = $db->prepare($sqlAdd);
		    $qryAdd->execute(array($project,$trans_no,$reporter,$type));
		    
		    $sel_ticket = "SELECT MAX(ticket_id) FROM tickets WHERE project = ? AND transaction_id = ? AND reporter_id = ?";
		    ...
		    if (!$_FILES['attachment']['error']) {
		        ...
		        $target = $currentdir . '../../attachment/' . $filename;
			      ...
			      $add_dtails = "INSERT INTO ticket_details(ticket_id,problem_subject,problem_desc,attachment,issue_status)VALUES(?,?,?,?,?)";
			      $stmt_dtails = $db->prepare($add_dtails);
			      $stmt_dtails->execute(array($row_ticket[0],$subject,$desc,$default,1));
